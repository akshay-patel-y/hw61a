...
def taxicab(a, b):
    ...
    """
    "*** YOUR CODE HERE ***"
    x = (abs(intersection(street(a),avenue(a))-intersection(street(b),avenue(b))))
    print(w(x))
...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t):
        if label(t) == old:
         return tree(new)
    x = [replace_leaf(b,old,new) for b in branches(t)]
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,attempt):
        nonlocal balance
        if len(atm)== 3:
                 return 'Your account is locked. Attempts: ' + str(atm)
        if attempt != password:
            atm.append(attempt)
